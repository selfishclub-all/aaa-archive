...
Step 1: sync-content.sh 실행
    ...
    │   (이미지 파일명 공백→언더스코어 변환)
    │   (Obsidian ![image](/images/image) → 표준 마크다운 이미지 변환)
    ▼
...
Obsidian에서 이미지를 붙여넣으면 `![스크린샷.png](/images/스크린샷.png)` 형태로 삽입된다. 이건 Obsidian 앱 안에서만 통하는 전용 문법이다. 웹 브라우저는 이 문법을 모르기 때문에 홈페이지에서는 이미지가 안 보이고 깨진 텍스트만 나온다.
